...
Um dem Benutzer eine möglichst einfache Verwendung des Containers zu ermöglichen, lässt sich mit \texttt{ENTRYPOINT} das auszuführende Programm festlegen und mit \texttt{CMD} Standardparameter an dieses übergeben.
...
Mit der Weiterentwicklung von Docker, beinhaltet die Kommandozeilenschnittstelle mittlerweile über 40 Kommandos. Mit der Version 1.13 wurde die CLI nun neu strukturiert.
In \autocite{docker-1.13:online} werden die neuen Management-Kommandos beschrieben.
Bei einem Aufruf von Docker ohne Parameter wird eine Übersicht über die Kommandos geliefert.
Diese Kommandos sind seit 1.13 ihren jeweiligen Komponenten im Docker-System zugewiesen und über diese erreichbar.
...
In \cref{lst:docker-build-example} wird ein Beispielkommando für das Erstellen des Dockerfiles aus \cref{lst:dockerfile-apache} angegeben.
...
Die häufigste Aufgabe eines Entwicklers bei der Arbeit mit Docker ist das Starten eines Containers.
...
Dadurch wird die Grundidee der "`verwerflichen"' Docker-Container unterstützt und das System sauber gehalten.
...
Änderung des Quelltextes am Host werden auch in den Container weitergeleitet.
...
In \cref{lst:docker-prune-example} wird das Ergebnis dieser Kommandos dargestellt, nachdem ein gestoppter Docker-Container im System vorhanden war.
\begin{verbatim}
